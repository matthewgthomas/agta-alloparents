...
fit <- lm(freq ~ agediff, data=childcare)
summary(fit)
...
childcare$youngadult <- ifelse(childcare$giverage >= 10 & childcare$agediff <= 20, c("1"), c("0")) 
...
fit <- lm(freq ~ agediff*agegroup, data=childcare)
